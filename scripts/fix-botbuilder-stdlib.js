...
const mainTargetDir = path.join(__dirname, '../node_modules/botbuilder-stdlib/lib/azureCoreHttpCompat');
...
const nestedTargetDir = path.join(__dirname, '../node_modules/botbuilder/node_modules/botbuilder-stdlib/lib/azureCoreHttpCompat');
fixBotbuilderStdlib(nestedTargetDir, 'botbuilder/node_modules/botbuilder-stdlib/lib/azureCoreHttpCompat/index.js');
...
const nestedStdlibDir = path.join(__dirname, '../node_modules/botbuilder/node_modules/botbuilder-stdlib');
...
if (fs.existsSync(nestedStdlibDir) && !fs.existsSync(topLevelStdlibDir)) {
  ...
  try {
    ...
    if (process.platform === 'win32') {
      execSync(`mklink /J "${topLevelStdlibDir}" "${nestedStdlibDir}"`, { shell: true, windowsHide: true });
    } else {
    ...
    try {
      execSync(`xcopy "${nestedStdlibDir}" "${topLevelStdlibDir}" /E /I /H /Y`, { shell: true, windowsHide: true });
